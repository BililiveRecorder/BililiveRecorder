...
namespace BililiveRecorder.Core
{
    internal class Room : IRoom
    {
        ...
        private void RecordTask_RecordFileOpening(object? sender, RecordFileOpeningEventArgs e)
        {
            ...
            {
                var extension = this.RoomConfig.DanmakuFileFormat == DanmakuFileFormat.Jsonl ? "jsonl" : "xml";
                this.basicDanmakuWriter.EnableWithPath(Path.ChangeExtension(e.FullPath, extension), this);
            }
            else
                this.basicDanmakuWriter.Disable();

            if (this.RoomConfig.SaveRawDanmaku)
                this.rawDanmakuWriter.EnableWithPath(Path.ChangeExtension(e.FullPath, "jsonl"), this);
            else
                this.rawDanmakuWriter.Disable();
        ...
        private void DanmakuClient_DanmakuReceived(object? sender, Api.Danmaku.DanmakuReceivedEventArgs e)
        {
            ...
            _ = Task.Run(async () => await this.rawDanmakuWriter.WriteAsync(d));
        }
